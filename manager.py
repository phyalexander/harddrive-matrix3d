...
class Matrix3d(abc.ABC):
    ...
    def __getitem__(self, *index: int) -> float:
        if len(index) == 1:
            if (0 > index[0]) or (index > self._shape[0]):
                return self._cache[index[0]]
            else:
                raise IndexError(f'Index {index[0]} is out of bounds [0..{self._shape[0]}]')
        elif len(index) == 2:
